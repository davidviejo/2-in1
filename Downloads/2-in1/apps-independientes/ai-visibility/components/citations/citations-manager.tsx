...
export function CitationsManager() {
  ...
  async function loadRows(projectId: string, page: number) {
    const response = await fetch(`/api/projects/${projectId}/citations?page=${page}&pageSize=${pagination.pageSize}`, { cache: 'no-store' });
    ...
    const data = (await response.json()) as { citations: CitationRow[]; pagination: Pagination };
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="overflow-x-auto rounded-md border border-slate-200 bg-white">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((row) => (
              <tr key={row.id}>
                ...
                <td className="px-2 py-1 text-slate-700">{row.model}</td>
                <td className="px-2 py-1 text-slate-700">{row.sourceDomain}</td>
                <td className="px-2 py-1 text-slate-700">
                  <a className="text-blue-700 underline" href={row.sourceUrl} rel="noreferrer" target="_blank">
                    {row.sourceUrl}
                  </a>
                </td>
        ...
        {rows.length === 0 ? <p className="px-3 py-3 text-xs text-slate-500">No citations available.</p> : null}
      </section>
