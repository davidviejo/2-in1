...
describe('seoChecklistStorage', () => {
  ...
  it('loads from IndexedDB first when idb backend is marked to avoid stale local copies', async () => {
    const staleLocalPages = [{ id: 'local-stale' }] as unknown as SeoPage[];
    const indexedDbPages = [{ id: 'idb-fresh' }] as unknown as SeoPage[];
    localStorage.setItem(storageKey, JSON.stringify(staleLocalPages));
    localStorage.setItem(markerKey, 'idb');

    await persistSeoChecklistPages(storageKey, indexedDbPages);

    await expect(loadSeoChecklistPages(storageKey)).resolves.toEqual(indexedDbPages);
  });

  it('loads from IndexedDB even when marker was not persisted', async () => {
    const pages = [{ id: 'idb-without-marker' }] as unknown as SeoPage[];
    const originalSetItem = Storage.prototype.setItem;
    vi.spyOn(Storage.prototype, 'setItem').mockImplementation(function (key: string, value: string) {
      if (key === storageKey || key === markerKey) {
        throw new DOMException('Quota exceeded', 'QuotaExceededError');
      }
