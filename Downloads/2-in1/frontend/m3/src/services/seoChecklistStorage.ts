...
export const loadSeoChecklistPages = async (storageKey: string): Promise<SeoPage[] | null> => {
  const preferredBackend = getStorageBackend(storageKey);

  if (preferredBackend === 'idb' && canUseIndexedDb()) {
    try {
      const indexedDbPages = await readFromIndexedDb(storageKey);
      if (indexedDbPages) return indexedDbPages;
    } catch (error) {
      console.warn('No se pudo leer SEO checklist en IndexedDB (backend preferido).', {
        storageKey,
        error,
      });
    }
