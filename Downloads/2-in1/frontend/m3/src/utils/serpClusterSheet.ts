...
export const buildTopSerpClusterWorkbook = (workbook: XLSX.WorkBook, topUrlLimit: number) => {
  ...
  let rows = XLSX.utils.sheet_to_json<unknown[]>(sheet, { header: 1, defval: '' });
  if (rows.length < 2) {
    throw new Error('El sheet no contiene filas de clusterización para reprocesar.');
  }

  let headers = rows[0].map((header) => String(header));
  const hasInlineSerpUrls = findHeaderIndex(headers, CLUSTER_URLS_COLUMN) !== -1;
  const hasBackendExportSheets =
    workbook.SheetNames.includes('Estrategia') && workbook.SheetNames.includes('URLs');

  if (!hasInlineSerpUrls && hasBackendExportSheets) {
    const urlsRows = XLSX.utils.sheet_to_json<unknown[]>(workbook.Sheets.URLs, {
      header: 1,
      defval: '',
    });
    const urlsHeaders = (urlsRows[0] || []).map((header) => String(header));
    const urlClusterIdIndex = findHeaderIndex(urlsHeaders, CLUSTER_ID_COLUMN);
    const urlIndex = findHeaderIndex(urlsHeaders, 'URL');
    const rankIndex = findHeaderIndex(urlsHeaders, 'Rank');
    const strategyClusterIdIndex = findHeaderIndex(headers, CLUSTER_ID_COLUMN);

    if (urlClusterIdIndex !== -1 && urlIndex !== -1 && strategyClusterIdIndex !== -1) {
      const urlsByCluster = new Map<string, Array<{ rank: number; url: string }>>();
      ...
      headers = [...headers, CLUSTER_URLS_COLUMN];
      rows = [
        headers,
