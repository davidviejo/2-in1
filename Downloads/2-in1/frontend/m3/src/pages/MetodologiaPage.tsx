...
import { useMetodologiaModules, useMetodologiaPhases, useMetodologiaResources } from '@/hooks/useMetodologia';
import { buildMetodologiaKPIs, getResourcesByTab, metodologiaTabs } from '@/lib/metodologia/transforms';

const MetodologiaPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<(typeof metodologiaTabs)[number]>('Documentación');
  ...
  useEffect(() => {
    if (!expandedModuleId && modules.length) setExpandedModuleId(modules[0].id);
  }, [expandedModuleId, modules]);

  const kpis = useMemo(() => buildMetodologiaKPIs(modules, phases, resources), [modules, phases, resources]);
  ...
  const filteredResources = useMemo(() => getResourcesByTab(resources, activeTab), [resources, activeTab]);
            ...
            <tbody>
              {phases.map((phase) => (
                <tr key={phase.id} className="border-b border-slate-100 align-top">
                  <td className="px-3 py-3 text-slate-700">{phase.title}</td>
                  <td className="px-3 py-3 text-slate-700">{phase.objective}</td>
                  <td className="px-3 py-3 text-slate-700">{phase.keyActions.join(' · ')}</td>
                  <td className="px-3 py-3 text-slate-700">{phase.deliverables.join(' · ')}</td>
                  <td className="px-3 py-3 text-slate-700">{phase.owner}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {resources.map((resource) => (
                <tr key={resource.id} className="border-b border-slate-100 align-top">
                  <td className="px-3 py-3"><span className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700"><NotebookText size={13} />{resource.source}</span></td>
                  ...
                  <td className="px-3 py-3 text-slate-600">{resource.lastUpdate}</td>
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2 text-slate-500">
                      <button type="button" title="Abrir" onClick={() => openResource(resource.title)}><ExternalLink size={14} /></button>
                      <button type="button" title="Compartir" onClick={() => void shareResource(resource.title)}><Share2 size={14} /></button>
                      <button type="button" title="Copiar enlace" onClick={() => void copyResource(resource.title)}><Copy size={14} /></button>
                      <button type="button" title="Más" onClick={() => info('Más opciones', `Acción: más opciones para "${resource.title}".`)}><MoreHorizontal size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
