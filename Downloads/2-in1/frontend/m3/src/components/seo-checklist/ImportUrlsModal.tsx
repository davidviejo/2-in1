...
const getMappedColumnIndex = (
  ...
  const direct = parsedHeaders.metadataColumnsByField.get(field);
  if (typeof direct === 'number') return direct;

  // Compatibilidad con layout de copiar/pegar:
  // URL | KW Principal | KWs Secundarias | Tipo | Cluster | Clics | Impresiones | Posición media | ...
  const secondaryKeywordsIndex = parsedHeaders.metadataColumnsByField.get('secondaryKeywords');
  if (typeof secondaryKeywordsIndex === 'number') {
    if (field === 'pageType') return secondaryKeywordsIndex + 1;
    if (field === 'cluster') return secondaryKeywordsIndex + 2;
    if (field === 'position') return secondaryKeywordsIndex + 5;
  }
