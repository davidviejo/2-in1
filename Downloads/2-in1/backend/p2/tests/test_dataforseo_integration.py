...
class TestDataForSEOIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_get_patcher = patch('apps.tools.scraper_core.requests.get')
    ...
    def test_smart_serp_search_uses_dataforseo_auto(self):
        # Mock DataForSEO task_post response
        task_post_response = MagicMock()
        task_post_response.json.return_value = {
            'status_code': 20000,
            'tasks': [{'id': 'task-123'}]
        }
        self.mock_post.return_value = task_post_response

        # Mock DataForSEO task_get response
        ...
        self.mock_get.return_value = task_get_response
